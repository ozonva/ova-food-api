...
var _ = Describe("Api", func() {
	...
	JustBeforeEach(func() {
		apiTest = api.NewFoodAPI(repoTest, chunkSize, producerMock)
	})
	AfterEach(func() {
		sqlMock.ExpectClose()
		db.Close()
	...
	Context("add coffee", func() {
		...
		It("add coffee", func() {
			...
			sqlMock.ExpectExec(regexp.QuoteMeta("INSERT INTO food_info (user_id,type,name,portion_size) VALUES ($1,$2,$3,$4)")).
				WithArgs(coffee.UserId, coffee.Type, coffee.Name, coffee.PortionSize).
				WillReturnResult(sqlmock.NewResult(0, 1))
			producerMock.EXPECT().Send(gomock.Any())
			func() {
	...
	Context("describe food", func() {
		...
		It("describe food", func() {
			sqlMock.ExpectQuery(regexp.QuoteMeta("SELECT id, user_id, type, name, portion_size FROM food_info WHERE id = $1")).
				WithArgs(coffee.Id).
	...
	Context("remove food", func() {
		...
		It("remove food", func() {
			sqlMock.ExpectExec(regexp.QuoteMeta("DELETE FROM food_info WHERE id = $1")).
	...
	Context("update food", func() {
		...
		It("update food", func() {
			...
			sqlMock.ExpectExec(regexp.QuoteMeta("UPDATE food_info SET id = $1, name = $2, portion_size = $3, type = $4, user_id = $5 WHERE id = $6")).
	...
	Context("multiadd foods", func() {
		...
		It("multiadd", func() {
			...
			sqlMock.ExpectExec(regexp.QuoteMeta("INSERT INTO food_info")).
				WithArgs(coffee.UserId, coffee.Type, coffee.Name, coffee.PortionSize).
	...
	Context("paging foods", func() {
		...
		It("paging foods", func() {
			sqlMock.ExpectQuery(regexp.QuoteMeta("SELECT * FROM food_info LIMIT 2 OFFSET 0")).
				WillReturnRows(
					sqlmock.NewRows([]string{"id", "user_id", "type", "name", "portion_size"}).
						AddRow(coffee.Id, coffee.UserId, coffee.Type, coffee.Name, coffee.PortionSize).
						AddRow(pizza.Id, pizza.UserId, pizza.Type, pizza.Name, pizza.PortionSize))
